...
export const _saveExternalBaptismItem = async (item: any): Promise<boolean> => {
  try {
    const token = getSessionToken() as string;
    await http(token, baseUrl).post(`/save-external-baptism`, item);
    return true;
  } catch (error) {
    return false;
  }
...
export const _getBaptismPerson = async (personId: number): Promise<any> => {
  try {
    const token = getSessionToken() as string;
