<template>
  ...
  >
    ...
    <LnxDataTable
      ...
      :headers="headersConfirmationParticipants"
    >
      <template
        v-slot:header="{
          request,
      ...
      >
        <v-card-item class="border-t border-b">
          <v-row class="d-flex justify-space-between align-center">
            ...
            >
              <v-btn
                v-permission="['crear_acta_confirmacion']"
              ...
              <v-btn
                v-permission="['agregar_participante_confirmacion']"
                prepend-icon="mdi-plus"
                class="me-2"
                :disabled="loading"
              >
                participante
                <ConfirmationParticipantForm @onSuccess="init" />
              </v-btn>
              <v-btn
                density="comfortable"
                icon="mdi-reload"
      ...
      <template v-slot:item.actions="{ item, loadDataTable }">
        <v-btn
          v-permission="[
            'editar_acta_confirmacion',
            'imprimir_acta_confirmacion',
            'anular_acta_confirmacion',
            'eliminar_acta_confirmacion',
          ]"
          icon
          size="small"
          variant="text"
        ...
        >
          ...
          <v-menu activator="parent">
            <v-list nav>
              <v-list-item
                v-permission="['editar_acta_confirmacion']"
              ...
              <v-list-item
                v-permission="['imprimir_acta_confirmacion']"
                v-if="item.status && item.issueDate !== null"
                title="Imprimir Acta"
                density="compact"
                value="print"
              ...
              <v-list-item
                v-permission="['anular_acta_confirmacion']"
                v-if="item.status && item.issueDate !== null"
                title="Anular"
                class="text-red"
                density="compact"
              ...
              <v-list-item
                v-permission="['eliminar_acta_confirmacion']"
...
import { headersConfirmationParticipants } from "@/app/modules/sacrament/constants/sacrament.constants.ts";

import ConfirmationParticipantForm from "@/app/modules/confirmation/components/ConfirmationParticipantForm.vue";
import DefineParameters from "@/app/modules/sacrament/components/DefineParameters.vue";

import FormAct from "@/app/modules/confirmation/components/FormAct.vue";
import PrintAct from "@/app/modules/confirmation/components/PrintAct.vue";
