<template>
  <v-card title="Personas" subtitle="Gestión de personas">
    <LnxDataTable :loadDataTable="_loadDataTable" :headers="headers">
      ...
      >
        <v-card-item class="border-t border-b">
          <v-row class="d-flex justify-space-between align-center">
            ...
            >
              <v-btn
                v-permission="['crear_persona']"
                class="me-2"
                :disabled="loading"
              >
                nuevo
                <Form @onSuccess="init" is-strict />
              </v-btn>
              <v-btn
                density="comfortable"
                icon="mdi-reload"
      ...
      <template v-slot:item.actions="{ item, loadDataTable }">
        <v-btn
          v-permission="['editar_persona', 'eliminar_persona']"
          icon
          size="small"
          variant="text"
        >
          ...
          <v-menu activator="parent">
            <v-list nav>
              ...
              >
                ...
                <Form :formState="item" @onSuccess="loadDataTable" is-strict />
              </v-list-item>
              <v-list-item
                v-permission="['eliminar_persona']"
                title="Eliminar"
                class="text-red"
                density="compact"
                value="delete"
